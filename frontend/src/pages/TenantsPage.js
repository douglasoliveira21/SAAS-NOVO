...
import { Plus, Building2, Link, Trash2, Settings, Pencil } from 'lucide-react';
...
export default function TenantsPage() {
  ...
  const handleEdit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.patch(`/tenants/${editTenant.id}`, { name: editTenant.name });
    ...
    <div>
                ...
                <tr key={t.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      {isAdmin && (
                        <>
                          <button className="btn btn-ghost btn-sm" onClick={() => setEditTenant({ id: t.id, name: t.name })} title="Editar nome">
                            <Pencil size={14} />
                          </button>
      ...
      {editTenant && (
        <div className="modal-overlay" onClick={() => setEditTenant(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2 className="modal-title">Editar Nome da Instância</h2>
            <form onSubmit={handleEdit}>
              <div className="form-group">
                <label>Nome da Empresa</label>
                <input required value={editTenant.name} onChange={e => setEditTenant(t => ({ ...t, name: e.target.value }))} placeholder="Empresa XYZ" />
              </div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 24 }}>
                <button type="button" className="btn btn-ghost" onClick={() => setEditTenant(null)}>Cancelar</button>
                <button type="submit" className="btn btn-primary" disabled={saving}>
