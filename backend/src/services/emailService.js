...
function createTransport() {
  ...
  const secure = process.env.SMTP_SECURE === 'true'; // true = SSL/TLS (465), false = STARTTLS (587)
  return nodemailer.createTransport({
    host: process.env.SMTP_HOST,
    port,
    secure,
    requireTLS: !secure, // force STARTTLS when not using SSL
    tls: {
      ciphers: 'SSLv3',
      rejectUnauthorized: false, // allow self-signed certs (Office 365 compatible)
    },
    auth: {
